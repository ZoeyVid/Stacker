...
public final class main extends JavaPlugin {
  ...
  public void onEnable() {
    Bukkit.getConsoleSender()
        .sendMessage(main.getPrefix() + ChatColor.DARK_GREEN + "The Plugin was enabled!");
    loadConfig();
    commandRegistration();
  ...
  public void onDisable() {
    Bukkit.getConsoleSender()
        .sendMessage(main.getPrefix() + ChatColor.DARK_RED + "The Plugin was disabled!");
  }
  ...
  private void loadConfig() {
    if ((new File("plugins/Stacker/config.yml")).exists()) {
      ...
      Bukkit.getConsoleSender()
          .sendMessage(main.getPrefix() + ChatColor.WHITE + "Config file loaded!");
    } else {
      saveDefaultConfig();
      cfg = getConfig();
      cfg.options().copyDefaults(true);
      Bukkit.getConsoleSender()
          .sendMessage(main.getPrefix() + ChatColor.WHITE + "Create and loaded config file!");
    }
    ...
    if ((new File("plugins/Stacker/language/" + loadConfig.language() + ".yml")).exists()) {
      ...
      try {
        FileUtils.copyURLToFile(
            new URL(
                "https://raw.githubusercontent.com/SanCraftDev/Stacker/"
                    + loadConfig.updateChannel()
                    + "/languages/"
                    + loadConfig.language()
                    + ".yml"),
            new File("plugins/Stacker/language/" + loadConfig.language() + ".yml"));
      } catch (Exception e) {
        try {
          FileUtils.copyURLToFile(
              new URL(
          ...
          try {
            FileUtils.copyURLToFile(
                new URL(
                    "https://raw.githubusercontent.com/SanCraftDev/Stacker/Stable/languages/en.yml"),
                new File("plugins/Stacker/language/" + loadConfig.language() + ".yml"));
          } catch (Exception e2) {
            Bukkit.getConsoleSender()
                .sendMessage(
                    main.getPrefix() + ChatColor.RED + "Error while downloading language file!");
    ...
    if (loadConfig.autoUpdate()) {
      try {
        if (loadConfig.updateChannel().equalsIgnoreCase("master")) {
          ...
          FileUtils.copyURLToFile(
              new URL(
                  "https://ci.sancraft.dev/job/Stacker-Dev/lastSuccessfulBuild/artifact/target/stacker.jar"),
              new File("plugins/stacker.jar"));
        } else {
          File oldFile = new File("plugins/stacker.jar");
          if (oldFile.exists()) FileUtils.delete(oldFile);
          FileUtils.copyURLToFile(
              new URL(
                  "https://ci.sancraft.dev/job/Stacker/lastSuccessfulBuild/artifact/target/stacker.jar"),
              new File("plugins/stacker.jar"));
        }
      } catch (Exception e) {
        Bukkit.getConsoleSender()
            .sendMessage(main.getPrefix() + ChatColor.RED + "Error while updating the plugin!");
      }
    }
  }
