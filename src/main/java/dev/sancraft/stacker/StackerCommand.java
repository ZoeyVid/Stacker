  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    if (!(sender instanceof Player)) {
      Bukkit.getConsoleSender()
          .sendMessage(
              main.getPrefix() + ChatColor.DARK_RED + "This Command can only execute as a player!");
      return true;
    }
    ...
    if (args.length == 0) {
      if (player.hasPermission("stacker.use.self")) {
        ...
        player.sendMessage(
            main.getPrefix()
                + ChatColor.DARK_RED
                + loadLanguage.permissionError().replace("%permission%", "stacker.use.self"));
        return true;
      }
      ...
      if (args[0].equalsIgnoreCase("enable")
        ...
        if (player.hasPermission("stacker.help")) {
          ...
          player.sendMessage(
              main.getPrefix()
                  ...
                  + "/stacker "
                  + ChatColor.WHITE
                  + loadLanguage.helpNormal());
          player.sendMessage(
              main.getPrefix()
                  + ChatColor.YELLOW
                  + "/stacker toggle/enable/disable <player>"
                  + ChatColor.WHITE
                  + loadLanguage.helpToggle());
          player.sendMessage(
              main.getPrefix()
                  + ChatColor.YELLOW
                  + "/stacker immune <player>"
                  + ChatColor.WHITE
                  + loadLanguage.helpImmune());
          player.sendMessage(
              main.getPrefix()
                  + ChatColor.YELLOW
                  + "/stacker help"
                  + ChatColor.WHITE
                  + loadLanguage.helpHelp());
        } else {
          player.sendMessage(
              main.getPrefix()
                  + ChatColor.DARK_RED
                  + loadLanguage.permissionError().replace("%permission%", "stacker.help"));
        }
        return true;
        ...
        if (player.hasPermission("stacker.immune.self")) {
          ...
          player.sendMessage(
              main.getPrefix()
                  + ChatColor.DARK_RED
                  + loadLanguage.permissionError().replace("%permission%", "stacker.immune.self"));
        }
        return true;
      }
    } else {
      if (args[0].equalsIgnoreCase("enable")
        ...
        if (player.hasPermission("stacker.use.other")) {
          ...
          if (target == null) {
            player.sendMessage(
                main.getPrefix() + ChatColor.DARK_RED + loadLanguage.errorPlayerOnline());
          } else {
            if (main.getStackmode().contains(target)) {
              ...
              player.sendMessage(
                  main.getPrefix()
                      + ChatColor.WHITE
                      + loadLanguage.leaveStackmodeOther().replace("%player%", target.getName()));
              if (loadConfig.sendMessage()) {
                target.sendMessage(
                    main.getPrefix() + ChatColor.WHITE + loadLanguage.leaveStackmodeSelf());
              }
            } else {
              ...
              player.sendMessage(
                  main.getPrefix()
                      + ChatColor.WHITE
                      + loadLanguage.joinStackmodeOther().replace("%player%", target.getName()));
              if (loadConfig.sendMessage()) {
                target.sendMessage(
                    main.getPrefix() + ChatColor.WHITE + loadLanguage.joinStackmodeSelf());
              }
            }
          }
        } else {
          player.sendMessage(
              main.getPrefix()
                  + ChatColor.DARK_RED
                  + loadLanguage.permissionError().replace("%permission%", "stacker.use.other"));
        }
        return true;
      } else if (args[0].equalsIgnoreCase("immune")) {
        Player target = Bukkit.getPlayerExact(args[1]);
          ...
          if (player.hasPermission("stacker.immune.other")) {
            if (main.getDisabled().contains(target)) {
              ...
              player.sendMessage(
                  main.getPrefix()
                      + ChatColor.WHITE
                      + loadLanguage.leaveImmuneOther().replace("%player%", target.getName()));
              if (loadConfig.sendMessage()) {
                target.sendMessage(
                    main.getPrefix() + ChatColor.WHITE + loadLanguage.leaveImmuneSelf());
              }
            } else {
              ...
              player.sendMessage(
                  main.getPrefix()
                      + ChatColor.WHITE
                      + loadLanguage.joinImmuneOther().replace("%player%", target.getName()));
              if (loadConfig.sendMessage()) {
                target.sendMessage(
                    main.getPrefix() + ChatColor.WHITE + loadLanguage.joinImmuneSelf());
              }
            }
          } else {
            player.sendMessage(
                main.getPrefix()
                    ...
                    + loadLanguage
                        .permissionError()
                        .replace("%permission%", "stacker.immune.other"));
          }
        }
        return true;
      }
